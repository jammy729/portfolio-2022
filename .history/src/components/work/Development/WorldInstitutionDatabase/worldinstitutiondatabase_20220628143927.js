  ...
  return (
    <Box>
      <Container maxW='850px'>
        ...
        <Writing>
          <DescriptionText>
            Once registered as a member, user can use the username
            and password to login. Similarly like the registration, it receive
            from the value from the form filled and display error messages if
            not filled. When the forms are filled, it checks the database to see
            if the username and password exists, and if it exists, it grants
            login member functionalities, and if not, it prompts an error.'
          </DescriptionText>
        </Writing>
        />
        <WorkImage
